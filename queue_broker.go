...
func (b *broker) get(name string, timeout time.Duration) (string, bool) {
	...
	// Канал буферизованный (cap 1) — это принципиально, не оптимизация.
	// put доставляет сообщение этому получателю, посылая в ch ПОД мьютексом.
	// На небуферизованном канале send заблокировался бы до момента приёма — а в
	// гонке с таймаутом приёмника уже нет: get ушёл в ветку time.After и сам ждёт
	// мьютекс, который держит put. Вышла бы взаимная блокировка. Буфер же даёт put
	// положить сообщение и сразу отпустить мьютекс, а проснувшийся по таймауту get
	// заберёт его из буфера при повторной проверке (вложенный select ниже).
	// Размер ровно 1: канал у каждого ждущего свой, и put шлёт в него не больше
	// одного сообщения (доставил — и тут же убрал ждущего из очереди), больше не нужно.
