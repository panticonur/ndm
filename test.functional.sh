...
#
# Запуск:   bash test.functional.sh [PORT]   (PORT по умолчанию 8080)
...
# kill -0) trap сработает, когда этих переменных ещё нет, и из-за set -u
# раскрытие "$TEST6_L" аварийно прервало бы сам обработчик. Поэтому объявляем
# их заранее пустыми.
