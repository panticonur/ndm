...
# Если скрипт подключили через `source`/`.` (а не запустили как `bash …`), то set -u
# и прочие настройки утекли бы в текущую оболочку, а тест выполнился бы прямо в ней.
# Поэтому здесь, ДО set -u, перезапускаем себя в отдельном bash и выходим через
# return (не exit — иначе закрыли бы оболочку вызывающего). Путь к себе берём от
# самого скрипта (dirname "${BASH_SOURCE[0]}"), чтобы не зависеть от текущего каталога.
